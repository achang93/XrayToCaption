...
Uses Salesforce/blip-image-captioning-base as a dual encoder:
  - model.get_image_features()  →  image embeddings
  - model.get_text_features()   →  text embeddings
...
Key differences from BiomedCLIP version:
  - open_clip replaced with transformers BlipModel + BlipProcessor
  - model.visual  →  model.vision_model  (freeze target)
  - logit_scale is fixed (no learned temperature parameter in BlipModel)
...
BATCH_SIZE         = 64       # Reduced from 128 — BLIP encoder is heavier than BiomedCLIP
EPOCHS             = 8
LR                 = 3e-5     # Slightly higher than BiomedCLIP; BLIP text encoder benefits
WEIGHT_DECAY       = 0.2
WARMUP_STEPS_RATIO = 0.1
PATIENCE           = 3
TEMPERATURE        = 0.07     # Fixed temperature (BLIP has no learned logit_scale)
FREEZE_VISION      = True     # Keep pretrained visual features intact
MAX_TEXT_LENGTH    = 256
DEVICE             = "cuda" if torch.cuda.is_available() else "cpu"
SAVE_DIR           = "./blip_contrastive"
    ...
    for epoch in range(EPOCHS):
        ...
        for images, texts in pbar:
            ...
            with autocast(device_type="cuda"):
                image_features = model.get_image_features(pixel_values=images).pooler_output
        ...
        with torch.no_grad():
            for images, texts in tqdm(val_loader, desc=f"Epoch {epoch+1}/{EPOCHS} [Val]"):
                ...
                img_f = model.get_image_features(pixel_values=images).pooler_output
                txt_f = model.get_text_features(**text_inputs).pooler_output
...
def evaluate():
    ...
    with torch.no_grad():
        for images, reports in tqdm(loader, desc="Encoding test set"):
            ...
            i_f = model.get_image_features(pixel_values=images).pooler_output
            t_f = model.get_text_features(**tokens).pooler_output
